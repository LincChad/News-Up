...
const RandomArticle = ({ data }: { data: news }) => {
  return (
    <div className="flex justify-between gap-2 p-2 mb-2 border-b border-gray-300">
      ...
      <Image src={`${data?.urlToImage !== null ? data?.urlToImage : "/apple-touch-icon.png"}`} alt={data?.title} fill sizes="(max-width:768px) 50vw,(max-width:1200px) 100vw, 100vw" className="object-cover"/>
      </div>
